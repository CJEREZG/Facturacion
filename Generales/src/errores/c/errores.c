...
{12,4130,1,0,0,
5,0,0,1,716,0,6,70,0,0,11,11,0,1,0,1,3,0,0,1,3,0,0,1,3,0,0,1,3,0,0,1,3,0,0,1,3,
0,0,1,5,0,0,1,3,0,0,1,5,0,0,2,3,0,0,2,5,0,0,
64,0,0,2,88,0,4,518,0,0,3,1,0,1,0,2,5,0,0,2,3,0,0,1,3,0,0,
};
...

{
 ...
 sqlstm.stmt = "declare LS_regAnoProceso fa_anoproceso % ROWTYPE ; BEGIN LS_\
regAnoProceso . num_proceso := :lhNumProceso ; LS_regAnoProceso . cod_cliente \
:= NVL ( :lhCodCliente , -1 ) ; LS_regAnoProceso . cod_concepto := :ihCodConce\
pto ; LS_regAnoProceso . cod_producto := :shCodProducto ; LS_regAnoProceso . c\
od_ciclfact := :lhCodCiclFact ; LS_regAnoProceso . num_abonado := :lhNumAbonad\
o ; LS_regAnoProceso . des_proceso := NVL ( :szhDesProceso , 'MAIN' ) ; LS_reg\
AnoProceso . cod_anomalia := :ihCodAnomalia ; LS_regAnoProceso . obs_anomalia \
:= :szhObsAnomalia ; FA_GESTION_ERRORES_PG . Fa_InsertaAnomalia_Pr ( LS_regAno\
Proceso ) ; EXCEPTION WHEN OTHERS THEN :ihFlgError := 1 ; :szhDesError := SUBS\
TR ( SQLERRM , 1 , 100 ) ; END ;";
