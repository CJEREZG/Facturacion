...
479,0,0,16,132,0,9,723,0,0,10,10,0,1,0,1,5,0,0,1,5,0,0,1,5,0,0,1,5,0,0,1,5,0,0,
1,5,0,0,1,5,0,0,1,5,0,0,1,5,0,0,1,5,0,0,
534,0,0,16,0,0,13,730,0,0,2,0,0,1,0,2,5,0,0,2,5,0,0,
557,0,0,16,0,0,15,738,0,0,0,0,0,1,0,
};
...

{
       ...
       sqlcxt((void **)0, &sqlctx, &sqlstm, &sqlfpn);
	      ...
	      if(sqlca.sqlcode<0)
	      {
	      		fprintf(stderr,"ERROR ORACLE - PGG [%ld] \n", sqlca.sqlcode);
	      		return FALSE;
	      }	
	    	else	
	      	{
				...
				{
						strcpy(pstParamGener.szCodIdioma,"1"); /* PGG - 166196 - 29-03-2011 */
...
double fnCnvDouble(double d,int uso)
{
  ...
  /*sprintf(szTemp,"%.*f",pstParamGener.iNumDecimal, d );*/
	...
	if (uso == USOFACT)
  		{
  		/*sprintf(szTemp,"%.*f",pstParamGener.iNumDecimalFact, d + 0.0000001);AFGS - 102561*/
  		sprintf(szTemp,"%.*f",4, d + 0.0000001);
  		}
  		else
  		{
  		/*sprintf(szTemp,"%.*f",pstParamGener.iNumDecimal, d + 0.0000001);AFGS - 102561*/
  		sprintf(szTemp,"%.*f",0, d + 0.0000001);
  		}


  dvalor = (double)atof(szTemp);

  /*if (dvalor > -0.000001 && dvalor < 0.000001)*/ 	/* PGG SOPORTE - 71849 - 16-10-2008 */
  if (dvalor >= -0.0001 && dvalor <= 0.0001) 				/* PGG SOPORTE - 71849 - 16-10-2008 */                          
      dvalor = (double)0.0;                     
