...
O=:b0 and A.COD_PROCESO=B.COD_PROCESO) and A.COD_PROCPREC=C.COD_PROCESO)) PROC\
 where ((TRAZ.COD_CICLFACT(+)=:b1 and TRAZ.COD_PROCESO(+)=PROC.COD_PROCPREC) a\
nd TRAZ.HOST_ID is null ) order by PROC.COD_PROCESO            ";
...
CESO) and A.COD_PROCPREC=C.COD_PROCESO)) PROC where ((TRAZ.COD_CICLFACT(+)=:b2\
 and TRAZ.COD_PROCESO(+)=PROC.COD_PROCPREC) and NVL(TRAZ.HOST_ID,:b3)=trim(:b3\
...
{12,4130,1,0,0,
5,0,0,1,160,0,4,73,0,0,4,1,0,1,0,2,3,0,0,2,5,0,0,2,5,0,0,1,3,0,0,
36,0,0,2,607,0,9,152,0,0,2,2,0,1,0,1,3,0,0,1,3,0,0,
59,0,0,2,0,0,13,172,0,0,5,0,0,1,0,2,5,0,0,2,3,0,0,2,5,0,0,2,3,0,0,2,3,0,0,
94,0,0,2,0,0,15,208,0,0,0,0,0,1,0,
109,0,0,3,215,0,4,218,0,0,4,2,0,1,0,2,3,0,0,2,5,0,0,1,3,0,0,1,3,0,0,
140,0,0,4,179,0,3,280,0,0,3,3,0,1,0,1,3,0,0,1,3,0,0,1,3,0,0,
167,0,0,5,293,0,4,368,0,0,9,2,0,1,0,2,3,0,0,2,5,0,0,2,5,0,0,2,5,0,0,2,3,0,0,2,
3,0,0,2,3,0,0,1,3,0,0,1,3,0,0,
218,0,0,6,262,0,5,546,0,0,9,9,0,1,0,1,3,0,0,1,5,0,0,1,5,0,0,1,5,0,0,1,3,0,0,1,
3,0,0,1,3,0,0,1,3,0,0,1,3,0,0,
269,0,0,7,130,0,4,655,0,0,6,3,0,1,0,1,5,0,0,1,5,0,0,2,3,0,0,2,5,0,0,2,5,0,0,1,
3,0,0,
308,0,0,8,584,0,9,734,0,0,5,5,0,1,0,1,3,0,0,1,3,0,0,1,3,0,0,1,5,0,0,1,5,0,0,
343,0,0,8,0,0,13,754,0,0,5,0,0,1,0,2,5,0,0,2,3,0,0,2,5,0,0,2,3,0,0,2,3,0,0,
378,0,0,8,0,0,15,790,0,0,0,0,0,1,0,
393,0,0,9,221,0,4,800,0,0,5,3,0,1,0,2,3,0,0,2,5,0,0,1,3,0,0,1,3,0,0,1,5,0,0,
428,0,0,10,188,0,3,864,0,0,8,8,0,1,0,1,3,0,0,1,3,0,0,1,3,0,0,1,5,0,0,1,3,0,0,1,
3,0,0,1,3,0,0,1,5,0,0,
475,0,0,11,267,0,4,955,0,0,12,5,0,1,0,1,5,0,0,1,5,0,0,2,3,0,0,2,5,0,0,2,5,0,0,
2,5,0,0,2,3,0,0,2,3,0,0,2,3,0,0,1,3,0,0,1,3,0,0,1,5,0,0,
538,0,0,12,240,0,5,1123,0,0,10,10,0,1,0,1,3,0,0,1,5,0,0,1,5,0,0,1,5,0,0,1,3,0,
0,1,3,0,0,1,3,0,0,1,3,0,0,1,3,0,0,1,5,0,0,
593,0,0,13,120,0,4,1204,0,0,4,2,0,1,0,2,3,0,0,2,3,0,0,1,3,0,0,1,5,0,0,
};
...
CT where COD_CICLFACT=:b3";
    ...
    /* EXEC SQL DECLARE cFaProcTraza CURSOR FOR
        ...
        AND     TRAZ.HOST_ID IS NULL /oAFGS - 102853o/
        ORDER BY PROC.COD_PROCESO; */ 
...

{
         ...
         sqlstm.sqladtp = &sqladt;
         sqlstm.sqltdsp = &sqltds;
         sqlstm.stmt = "select TRAZ.COD_ESTAPROC ,PROC.DES_PROCESO into :b0,\
:b1  from FA_TRAZAPROC TRAZ ,FA_PROCFACT PROC where (((TRAZ.COD_CICLFACT=:b2 a\
nd TRAZ.COD_PROCESO=:b3) and TRAZ.COD_PROCESO=PROC.COD_PROCESO) and HOST_ID is\
 null )";
         sqlstm.iters = (unsigned int  )1;
...
s (:b0,:b1,:b2,sysdate,'Proceso Iniciado',0,0,0)";
        sqlstm.iters = (unsigned int  )1;
...
_TRAZAPROC where ((COD_CICLFACT=:b12 and COD_PROCESO=:b13) and HOST_ID is null\
 )";
    sqlstm.iters = (unsigned int  )1;
...
OCESO=:b3,COD_CLIENTE=:b4,NUM_ABONADO=:b5,NUM_REGISTROS=:b6 where ((COD_CICLFA\
CT=:b7 and COD_PROCESO=:b8) and HOST_ID is null )";
    sqlstm.iters = (unsigned int  )1;
...
AR(SYSDATE,:b0) into :b2,:b3,:b4  from FA_CICLFACT where COD_CICLFACT=:b5";
    ...
    /* EXEC SQL DECLARE cFaProcTrazaHost CURSOR FOR
        ...
        WHERE   TRAZ.COD_CICLFACT (+)  = :lhCiclParam
        AND     TRAZ.COD_PROCESO  (+)  = PROC.COD_PROCPREC
        AND     NVL(TRAZ.HOST_ID,:szhHostId) = TRIM(:szhHostId)
        ORDER BY PROC.COD_PROCESO; */ 
...
im(:b4))";
    ...
    else if (SQLCODE == SQLNOTFOUND)
    {
        /****************************************************************************/
        /*   Proceso NO se ha ejecutado aun para el ciclo de Facturacion.           */
        /*   Inserta Registro en la Tabla de Traza con Host Id.                     */
        /****************************************************************************/
        ihTrazCodEstaActual = iPROC_EST_RUN;
        vDTrazasLog  (szExeProceso, "\n\t**  Insertando Proceso de FA_TRAZAPROC  **"
                                    "\n\t\t=>  Ciclo de Facturacion     [%ld]"
                                    "\n\t\t=>  Codigo de Proceso        [%d]\n"
                                    "\n\t\t=>  Host Id.                 [%s]\n"                                    
                                    ,LOG03,lCiclParam,iCodProceso,szhHostId);
...
BOOL bfnSelectTrazaProcHost (long lCicloFac,int iCodProc, TRAZAPROC * pstTraza, char *szHostId)
{
    ...
    vDTrazasLog  (szExeProceso, "\n\t**  Parametros de Entrada a (bfnSelectTrazaProcHost) **"
                                    "\n\t\t=>  Ciclo de Facturacion     [%ld]"
                                    "\n\t\t=>  Codigo de Proceso        [%d]"
                                    "\n\t\t=>  Host Id.                 [%s]"                                    
                                    ,LOG05,lCicloFac,iCodProc,szHostId);
...
ACT=:b14 and COD_PROCESO=:b15) and HOST_ID=trim(:b16))";
    sqlstm.iters = (unsigned int  )1;
...
BOOL bfnUpdateTrazaProcHost (TRAZAPROC pstTraza, char *szHostId)
{
   ...
   /* EXEC SQL BEGIN DECLARE SECTION; */ 
    ...
    vDTrazasLog  (szExeProceso, "\n\t**  Parametros de Entrada a (bfnUpdateTrazaProcHost) **"
                                    "\n\t\t=>  Ciclo de Facturacion     [%ld]"
                                    "\n\t\t=>  Codigo de Proceso        [%d]"
                                    "\n\t\t=>  Host Id.                 [%s]"                                    
                                    ,LOG05,pstTraza.lCodCiclFact,pstTraza.iCodProceso,szHostId);
