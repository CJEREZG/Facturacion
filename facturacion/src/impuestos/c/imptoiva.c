...
BOOL bImptosIvaClienteGeneral (int iTipoFact)
{
    ...
    /******************************************************/
    /* Codigo Aplicar Retención de Impuestos              */
    /* SAAM-20060110, Proyecto P-TMC-05040 RETENCIÓN IVA  */
    /******************************************************/

    if (!bfnEvaluaClienteconRetencion(stCliente.lCodCliente))
    {
        vDTrazasLog(modulo,"\n\t\t# Cliente [%ld] => NO Aplica Retención de Impuestos\n",LOG03, stCliente.lCodCliente);
    }
    else
    {
        if (!bfnCargaConceptosConRetencion(&pstConcReten.stConcRetencion, &pstConcReten.iCantConcRetencion))
        {
            vDTrazasError(modulo,"\n\t** Error en Carga de Conceptos con Retención  **",LOG01);
            return (FALSE);
        }
        iNumRegIni = 0;
        iNumRegIni = stPreFactura.iNumRegistros;
        ...
        for (i=0;i<iNumRegIni;i++) /* for de revisión de conceptos */
        {
            vDTrazasLog(modulo, "\n\t\t* i ---> [%d]",LOG05, i);
            if (stPreFactura.A_PFactura.iCodTipConce [i] == ARTICULO)
            {
                ...
                {
                    vDTrazasLog(modulo,"\n\t\t# Concepto [%ld] => NO Aplica Retención de Impuestos\n",LOG03, stPreFactura.A_PFactura.iCodConcepto[i]);
                }
                else
                {
                    dImpConceptoAux = 0.0;
                    ...
                    for (j=0;j<iNumRegIni;j++) /* for de revisión de impuestos y descuentos --> antes decia i++... <>--*/
                    {
                        vDTrazasLog(modulo, "\n\t\t* Dentro de for(j)...:"
                                            "\n\t\t  j              : [%d]"
                    ...
                    } /* fin for de revisión de impuestos */
        ...
        } /* FIN for de revisión de conceptos */
    }
...
OD_CLIENTE=:b1 and SYSDATE between FEC_DESDE and FEC_HASTA)";
    ...
    if(SQLCODE != SQLOK)
    {
        vDTrazasError(szExeName,"\n\t=> Si el Cliente Aplica Retención [%ld]",LOG01,lhCodCliente);
        return FALSE;
    }
