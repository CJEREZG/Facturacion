...
,ORD_CUOTA ,IND_MODVENTA ,COD_TECNOLOGIA  from FA_PREFACTURA where NUM_PROCESO\
=:b2           ";
...
{12,4130,1,0,0,
5,0,0,1,715,0,265,186,0,0,3,3,0,1,0,1,5,0,0,1,5,0,0,1,3,0,0,
32,0,0,1,0,0,269,192,0,0,45,0,0,1,0,2,3,0,0,2,3,0,0,2,3,0,0,2,3,0,0,2,3,0,0,2,
...
5,0,0,
227,0,0,1,0,0,271,400,0,0,0,0,0,1,0,
};
...
BOOL bfnDBIFCompraMiscela (long lNumProceso)
{
  ...
  static char    szhCodTecnologia [8];/* EXEC SQL VAR szhCodTecnologia IS STRING(8); */ 
...

{
          ...
          sqlstm.sqhstv[44] = (unsigned char  *)szhCodTecnologia;
          sqlstm.sqhstl[44] = (unsigned long )8;
          sqlstm.sqhsts[44] = (         int  )0;
          sqlstm.sqindv[44] = (         short *)&i_shCodTecnologia;
          ...
          sqlcxt((void **)0, &sqlctx, &sqlstm, &sqlfpn);
             ...
             if (SQLCODE == SQLOK)
             {
                 ...
                 stPreFactura.A_PFactura.dImpFacturable[i] =
                      fnCnvDouble (dhImpFacturable, USOFACT)  ;
                 stPreFactura.A_PFactura.dImpMontoBase [i] =
                      fnCnvDouble (dhImpMontoBase , USOFACT)  ;
                 ...
                 stPreFactura.A_PFactura.lNumAbonado [i]   =
                                   /*(i_shNumAbonado == -1)?-1:lhNumAbonado ;73156 CJG se cambia el -1 por 0*/
                                   (i_shNumAbonado == -1)?0:lhNumAbonado ;
                 ...
                 if (stConcepto.sIndTecnologia == 1 && i_shCodTecnologia != ORA_NULL)
				 {
	                 strcpy (stPreFactura.A_PFactura.szCodTecnologia [i], szhCodTecnologia );
	             }
	             else
	             {
...

{
   ...
   if (stPreFactura.iNumRegistros > 0)
       strcpy (szSysDate, stPreFactura.A_PFactura.szFecValor [0]);
