                                 ...
                                 TRANSCONTADO *pstTransContado)
{
  ...
  char  szCodOficina   [3]="";
	...
	if (stPreFactura.iNumRegistros > 0)
	{
	    if (stPreFactura.A_PFactura.lNumTransaccion [0] > 0 )
	    {
			if ( stPreFactura.A_PFactura.lNumVenta [0] > 0 )
		    {
		        pstVenta->lNumVenta       = 
		                                stPreFactura.A_PFactura.lNumVenta      [0];
		        pstVenta->lNumTransaccion = 
		                                stPreFactura.A_PFactura.lNumTransaccion[0]; 
	    ...
	    stCliente.lCodCliente = stPreFactura.A_PFactura.lCodCliente [0]; 
	    stCliente.iIndCredito = stPreFactura.A_PFactura.iIndCuota   [0];
	    stCliente.iIndSuperTel= 0                                      ;
	    stCliente.iCodCatImpos= stPreFactura.A_PFactura.iCodCatImpos[0];
	    stCliente.lCodPlanCom = stPreFactura.A_PFactura.lCodPlanCom [0];
	    ...
	    if (!bfnEvalZonasImpos (szSysDate, 5, &iCodZonaImpos,0 ))  /* Se fuerza el valor a 0, PH-200407070262 VRG */ 
		{
