...
int main (int argc, char *argv[])
{
  ...
  fprintf (stderr,"\nExe->Carga_Inicial version " __DATE__ " " __TIME__ " TMC.\n" );
	...
	ifnInitORATh(szUser,szPass,&ciclo,factura,szSysDate);
	if(ciclo==-1){
...
int ifnCopiarShared(void)
{
	...
	if (!bfnCargarShared("FAC_CICLO"))
	{
    		fprintf(stdout, "\n\nERROR(ifnCopiarShared):"
      			"\nError al compartir datos de [FAC_CICLOCLI]"
      			"\nError: [%s]", strerror(iRet));
    		return FALSE;
	}
...
*/
	if (!bfnCargarShared("DETCARG_PUNTUALES"))
	{
    		fprintf(stdout, "\n\nERROR(ifnCopiarShared):"
      			"\nError al compartir datos de [DETCARG_PUNTUALES]"
      			"\nError: [%s]", strerror(iRet));
    		return FALSE;
	}
...
BOOL bfnCargarShared(char *szTable)
{
  	...
  	if (!strcmp("FAC_CICLO", szTable))
  	{
    		fprintf(stdout,"\n\t\tCopiando: FAC_CICLOCLI");
    		if( (iRet=ifnCreateShared(TaS.pDataTable, ID_FAC_CICLO,"FA_ABONADOS",
                    			 (TaS.lFacCiclo*sizeof(FAC_CICLO)),
...
*/  	
  	if (!strcmp("DETCARG_PUNTUALES", szTable))
  	{
    		fprintf(stdout,"\n\t\tCopiando: DETCARG_PUNTUALES");
    		if( (iRet=ifnCreateShared(TaS.pDataTable, ID_DETCARG_PUNTUALES,"DETCARG_PUNTUALES",
                    			 (TaS.lDescPuntuales*sizeof(DETCARG_PUNTUALES)),
                    			  TaS.lDescPuntuales,&TaS.lMemoriaTotal,
                    			 (TaS.SharedPerms | IPC_CREAT),
                    			 (void *) TaS.pstlDescPuntuales)) != SALIR_OK)
    		{
      			fprintf(stdout, "\n\nERROR(bfnCargarTablasShared):"
        			"\nError al compartir datos de [%s]"
...
void *fncDatos(	void* pArg)
{
	...
	/* se agrega la obtencion del numero de conceptos */
	if (!bfnObtCantConcep(&TaS.lGeConceptos,ctxDatos))
	{
		vfnLog(FILE_BOTH, "Error en bfnObtCantConcep");
		return (void*)FALSE;
	}
	if (TaS.lGeConceptos > 999999) /* se define limite */
	{
		vfnLog(FILE_BOTH, "Error el numero de conceptos excede el manejable");
		return (void*)FALSE;
	}
	/* ******************************************************************** */
	
	if (!bfnObtGeConceptos(&TaS.lGeConceptos,&TaS.pstlGeConceptos, ctxDatos))
	{
...
*/
	...
	if (!bfnObtGeCargosBasico(&TaS.lGeCargosBasico,&TaS.pstlGeCargosBasico, ctxDatos))
	{
	...
	if (!bfnObtGePlanTarif(&TaS.lGePlanTarif,&TaS.pstlGePlanTarif, ctxDatos))
	{
...
/* 	BOOL bFound=FALSE; */
	...
	if (!bfnObtFacClientes(&TaS.lFacClientes, &TaS.pstlFacClientes, ctxClientes, ciclo, lClienteIni, lClienteFin))
	{
...
*/
        if (!bfnObtDescPuntuales(&TaS.lDescPuntuales,&TaS.pstlDescPuntuales,ctxUnipac, ciclo))
	{
		vfnLog(FILE_BOTH, "Error en bfnObtDescPuntuales");
		return (void*)FALSE;
	}
/*
