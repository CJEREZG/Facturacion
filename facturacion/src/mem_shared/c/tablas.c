...
BOOL bfnObtGeConceptos ( long *lGeConceptos,CONCEPTO **pstlGeConc,void* ctx)
{
	int iOra;
	long lNumFilas;
	...
	vfnLog(FILE_LOG, "\n\t-Leyendo datos: CONCEPTO N° [%ld]", *lGeConceptos);
	...
	*pstlGeConc =(CONCEPTO*) realloc(*pstlGeConc,((*lGeConceptos)*sizeof(CONCEPTO))); /* se fija momentaneamente al tamaño de MAX_CONCEPTOS */
	...
	iOra= ifnOraDeclararGeConceptos ( ctx);
	if (iOra != SQL_OK)
	{
		vfnLog(FILE_BOTH, "\n\nERROR(bfnObtGeConceptos):"
	...
	memset(stlGeConcTemp, 0, sizeof(CONCEPTO)*(*lGeConceptos));
	...
	while (iOra != NOT_FOUND)
	{
		...
		for (lCont = 0 ; lCont < lNumFilas ; lCont++)
		{
			...
			stlGeConcTemp[lIndConc].sIndTecnologia = stlGeConcHost.sIndTecnologia[lCont];
			strcpy(stlGeConcTemp[lIndConc].szDefTecnologia,stlGeConcHost.szDefTecnologia[lCont]);
...
BOOL bfnObtGeLimCreditos ( long *lGeLimCreditos,LIMCREDITOS **pstlGeLimCreditos,void* ctx)
{
	...
	while (iOra != NOT_FOUND)
	{
		...
		for (lCont = 0 ; lCont < lNumFilas ; lCont++)
		{
			strcpy(pstlGeLimCreditosTemp[lCont].szRowid,stlGeLimCreditosHost.szRowid[lCont]);
			...
			strcpy(pstlGeLimCreditosTemp[lCont].szDesCredMor,stlGeLimCreditosHost.szDesCredMor[lCont]);
...
BOOL bfnObtGeActividades ( long *lGeActividades,ACTIVIDADES **pstlGeActividades,void* ctx)
{
	...
	while (iOra != NOT_FOUND)
	{
		...
		for (lCont = 0 ; lCont < lNumFilas ; lCont++)
		{
			strcpy(pstlGeActividadesTemp[lCont].szRowid,stlGeActividadesHost.szRowid[lCont]);
...
BOOL bfnObtGeProvincias ( long *lGeProvincias,PROVINCIAS **pstlGeProvincias,void* ctx)
{
	...
	while (iOra != NOT_FOUND)
	{
		...
		for (lCont = 0 ; lCont < lNumFilas ; lCont++)
		{
			strcpy(pstlGeProvinciasTemp[lCont].szRowid,stlGeProvinciasHost.szRowid[lCont]);
			...
			strcpy(pstlGeProvinciasTemp[lCont].szDesProvincia,stlGeProvinciasHost.szDesProvincia[lCont]);
		}
...
BOOL bfnObtGeRegiones ( long *lGeRegiones,REGIONES **pstlGeRegiones,void* ctx)
{
	...
	while (iOra != NOT_FOUND)
	{
		...
		for (lCont = 0 ; lCont < lNumFilas ; lCont++)
		{
			strcpy(pstlGeRegionesTemp[lCont].szRowid,stlGeRegionesHost.szRowid[lCont]);
...
BOOL bfnObtGeCatImpositiva ( long *lGeCatImpositiva,CATIMPOSITIVA **pstlGeCatImpositiva,void* ctx)
{
	...
	while (iOra != NOT_FOUND)
	{
		...
		for (lCont = 0 ; lCont < lNumFilas ; lCont++)
		{
			strcpy(pstlGeCatImpositivaTemp[lCont].szRowid,stlGeCatImpositivaHost.szRowid[lCont]);
...
BOOL bfnObtGeZonaImpositiva ( long *lGeZonaImpositiva,ZONAIMPOSITIVA **pstlGeZonaImpositiva,void* ctx)
{
	...
	while (iOra != NOT_FOUND)
	{
		...
		{
			vfnLog(FILE_BOTH, "\n\nERROR(bfnObtGeZonaImpositiva):"
			"\n\tError al  realizar el FETCH sobre ZONAIMPOSITIVA [%d]", iOra);
			vfnImpErrorORACLE();
			return FALSE;
		}
		...
		for (lCont = 0 ; lCont < lNumFilas ; lCont++)
		{
			strcpy(pstlGeZonaImpositivaTemp[lCont].szRowid,stlGeZonaImpositivaHost.szRowid[lCont]);
...
BOOL bfnObtGeImpuestos ( long *lGeImpuestos,IMPUESTOS **pstlGeImpuestos,void* ctx)
{
	...
	while (iOra != NOT_FOUND)
	{
		...
		for (lCont = 0 ; lCont < lNumFilas ; lCont++)
		{
			strcpy(pstlGeImpuestosTemp[lCont].szRowid,stlGeImpuestosHost.szRowid[lCont]);
...
BOOL bfnObtGeTipImpues ( long *lGeTipImpues,TIPIMPUES **pstlGeTipImpues,void* ctx)
{
	int iOra;
	long lNumFilas;
	static TIPIMPUES_HOST stlGeTipImpuesHost;
	TIPIMPUES *pstlGeTipImpuesTemp;
	long lCont;

	
	 *lGeTipImpues = 0;
	 *pstlGeTipImpues = NULL;
	
	vfnLog(FILE_LOG, "\n\t-Leyendo datos: TIPIMPUES");
	iOra= ifnOraDeclararGeTipImpues (ctx );
	if (iOra != SQL_OK)
	{
		vfnLog(FILE_BOTH, "\n\nERROR(bfnObtGeTipImpues):"
		"\n\tError al declarar cursor sobre TIPIMPUES");
		vfnImpErrorORACLE();
		return FALSE;
	}
	...
	while (iOra != NOT_FOUND)
	{
		iOra= ifnOraLeerGeTipImpues(&stlGeTipImpuesHost,&lNumFilas,ctx);
		if (iOra != SQL_OK  && iOra != NOT_FOUND)
		{
			vfnLog(FILE_BOTH, "\n\nERROR(bfnObtGeTipImpues):"
			"\n\tError al  realizar el FETCH sobre TIPIMPUES");
			vfnImpErrorORACLE();
			return FALSE;
		}
		 ...
		 *pstlGeTipImpues =(TIPIMPUES*) realloc(*pstlGeTipImpues,(((*lGeTipImpues)+lNumFilas)*sizeof(TIPIMPUES)));
		...
		pstlGeTipImpuesTemp = &(*pstlGeTipImpues)[(*lGeTipImpues)];
		memset(pstlGeTipImpuesTemp, 0, sizeof(TIPIMPUES)*lNumFilas);
		for (lCont = 0 ; lCont < lNumFilas ; lCont++)
		{
			strcpy(pstlGeTipImpuesTemp[lCont].szRowid,stlGeTipImpuesHost.szRowid[lCont]);
			pstlGeTipImpuesTemp[lCont].iCodTipImpue = stlGeTipImpuesHost.iCodTipImpue[lCont];
			strcpy(pstlGeTipImpuesTemp[lCont].szDesTipImpue,stlGeTipImpuesHost.szDesTipImpue[lCont]);
			pstlGeTipImpuesTemp[lCont].dImpUmbral = stlGeTipImpuesHost.dImpUmbral[lCont];		

		}
		
		(*lGeTipImpues) += lNumFilas;
	}
	vfnLog(FILE_LOG, "\t\t\t[%d]", *lGeTipImpues);
	
	iOra=ifnOraCerrarGeTipImpues(ctx);
	if (iOra != SQL_OK)
	{
		vfnLog(FILE_BOTH, "\n\nERROR(bfnObtGeTipImpues):"
		"\n\tError al cerrar el cursor");
	...
	qsort((void*)*pstlGeTipImpues, *lGeTipImpues, sizeof(TIPIMPUES),ifnCompareGeTipImpues);
...
BOOL bfnObtGeConversion ( long *lGeConversion,CONVERSION **pstlGeConversion,void* ctx)
{
	...
	while (iOra != NOT_FOUND)
	{
		...
		for (lCont = 0 ; lCont < lNumFilas ; lCont++)
		{
			/*strcpy(pstlGeConversionTemp[lCont].szRowid,stlGeConversionHost.szRowid[lCont]);*/
			...
			/*strcpy(pstlGeConversionTemp[lCont].szCodUsuario,stlGeConversionHost.szCodUsuario[lCont]);*/
		}
...
BOOL bfnObtGeDocumSucursal ( long *lGeDocumSucursal,DOCUMSUCURSAL **pstlGeDocumSucursal,void* ctx)
{
	...
	while (iOra != NOT_FOUND)
	{
		...
		for (lCont = 0 ; lCont < lNumFilas ; lCont++)
		{
			strcpy(pstlGeDocumSucursalTemp[lCont].szRowid,stlGeDocumSucursalHost.szRowid[lCont]);
...
BOOL bfnObtGeLetras ( long *lGeLetras,LETRAS **pstlGeLetras,void* ctx)
{
	...
	while (iOra != NOT_FOUND)
	{
		...
		for (lCont = 0 ; lCont < lNumFilas ; lCont++)
		{
			strcpy(pstlGeLetrasTemp[lCont].szRowid,stlGeLetrasHost.szRowid[lCont]);
...
BOOL bfnObtGeGrupoCob ( long *lGeGrupoCob,GRUPOCOB **pstlGeGrupoCob,void* ctx, int ipCiclo)
{
	int iOra;
	long lNumFilas;
	...
	iOra= ifnOraDeclararGeGrupoCob ( ctx, ipCiclo);
	if (iOra != SQL_OK)
	{
	...
	while (iOra != NOT_FOUND)
	{
		...
		for (lCont = 0 ; lCont < lNumFilas ; lCont++)
		{
			...
			pstlGeGrupoCobTemp[lCont].iCodCiclo = ipCiclo;
			...
			pstlGeGrupoCobTemp[lCont].iTipCobroAnt = stlGeGrupoCobHost.iTipCobroAnt[lCont];
...
BOOL bfnObtGeTarifas ( long *lGeTarifas,TARIFAS **pstlGeTarifas,void* ctx)
{
	...
	while (iOra != NOT_FOUND)
	{
		...
		for (lCont = 0 ; lCont < lNumFilas ; lCont++)
		{
/*			strcpy(pstlGeTarifasTemp[lCont].szRowid,stlGeTarifasHost.szRowid[lCont]);
			pstlGeTarifasTemp[lCont].iCodProducto = stlGeTarifasHost.iCodProducto[lCont];
			strcpy(pstlGeTarifasTemp[lCont].szCodActAbo,stlGeTarifasHost.szCodActAbo[lCont]);*/
...
BOOL bfnObtGeActuaServ ( long *lGeActuaServ,ACTUASERV **pstlGeActuaServ,void* ctx)
{
	...
	while (iOra != NOT_FOUND)
	{
		...
		for (lCont = 0 ; lCont < lNumFilas ; lCont++)
		{
/*			strcpy(pstlGeActuaServTemp[lCont].szRowid,stlGeActuaServHost.szRowid[lCont]);
			pstlGeActuaServTemp[lCont].iCodProducto = stlGeActuaServHost.iCodProducto[lCont];
			strcpy(pstlGeActuaServTemp[lCont].szCodActAbo,stlGeActuaServHost.szCodActAbo[lCont]);*/
...
BOOL bfnObtGeCuadCtoPlan ( long *lGeCuadCtoPlan,CUADCTOPLAN **pstlGeCuadCtoPlan,void* ctx)
{
	...
	while (iOra != NOT_FOUND)
	{
		...
		for (lCont = 0 ; lCont < lNumFilas ; lCont++)
		{
			strcpy(pstlGeCuadCtoPlanTemp[lCont].szRowid,stlGeCuadCtoPlanHost.szRowid[lCont]);
...
/******************************************************************************
Funcion         :       bfnObtDescPuntuales
*******************************************************************************/

BOOL bfnObtDescPuntuales (long *lDescPuntuales,DETCARG_PUNTUALES **pstlDescPuntuales, void* ctx, long ciclo)
{
	int iOra;
	int iInd = 0;
	long lNumFilas;
	static DETCARG_PUNTUALES_HOST stlDescPuntualesHost;
	DETCARG_PUNTUALES *pstlDescPuntualesTemp;
	long lCont;
	
	
	 *lDescPuntuales = 0;
	 *pstlDescPuntuales = NULL;
	
	vfnLog(FILE_LOG, "\n\t-Leyendo datos: DETCARG_PUNTUALES");
	iOra=ifnOraDeclararDescCargPunt (ctx, ciclo);
	if (iOra != SQL_OK)
	{
		vfnLog(FILE_BOTH, "\n\nERROR(bfnObtDescPuntuales):"
		"\n\tError al declarar cursor sobre DETCARG_PUNTUALES");
		vfnImpErrorORACLE();
		return FALSE;
	}
	...
	while (iOra != NOT_FOUND)
	{
		iOra= ifnOraLeerDescCargPunt(&stlDescPuntualesHost,&lNumFilas,ctx);
		if (iOra != SQL_OK  && iOra != NOT_FOUND)
		{
			vfnLog(FILE_BOTH, "\n\nERROR(bfnObtDescPuntuales):"
			"\n\tError al  realizar el FETCH sobre DETCARG_PUNTUALES");
			vfnImpErrorORACLE();
			return FALSE;
		}
	 	...
	 	*pstlDescPuntuales =(DETCARG_PUNTUALES*) realloc(*pstlDescPuntuales,(((*lDescPuntuales)+lNumFilas)*sizeof(DETCARG_PUNTUALES)));
	
		if (!*pstlDescPuntuales)
		{
			vfnLog(FILE_BOTH, "\n\nERROR(bfnObtDescPuntuales):"
			"\n\tError al alocar memoria"
			"\n\tError: [%d][%s]", errno, strerror(errno));
		...
		pstlDescPuntualesTemp = &(*pstlDescPuntuales)[(*lDescPuntuales)];
		memset(pstlDescPuntualesTemp, 0, sizeof(DETCARG_PUNTUALES)*lNumFilas);
		for (lCont = 0 ; lCont < lNumFilas ; lCont++)
		{
			pstlDescPuntualesTemp[lCont].lCodCliente   = stlDescPuntualesHost.lCodCliente[lCont];
			pstlDescPuntualesTemp[lCont].lNumAbonado   = stlDescPuntualesHost.lNumAbonado[lCont];
			pstlDescPuntualesTemp[lCont].iCodTipDescu  = stlDescPuntualesHost.iCodTipDescu[lCont];
			pstlDescPuntualesTemp[lCont].lCodConcCargo = stlDescPuntualesHost.lCodConcCargo[lCont];
			pstlDescPuntualesTemp[lCont].lCodConcDesc  = stlDescPuntualesHost.lCodConcDesc[lCont];
			pstlDescPuntualesTemp[lCont].dMtoDcto      = stlDescPuntualesHost.dMtoDcto[lCont]; 
			pstlDescPuntualesTemp[lCont].dMtoAplicado  = stlDescPuntualesHost.dMtoAplicado[lCont];
			strcpy(pstlDescPuntualesTemp[lCont].szRowid,stlDescPuntualesHost.szRowid[lCont]); 
		}
	
		(*lDescPuntuales) += lNumFilas;
	}
	vfnLog(FILE_LOG, "\t\t\t[%d]", *lDescPuntuales);

	iOra=ifnOraCerrarDescPuntuales(ctx);
	if (iOra != SQL_OK)
	{
		vfnLog(FILE_BOTH, "\n\nERROR(bfnObtDescPuntuales):"
		"\n\tError al cerrar el cursor");
	...
	//qsort((void*)*pstlDescPuntuales, *lDescPuntuales, sizeof(DETCARG_PUNTUALES),ifnCmpDescPuntuales); INC 176817
...
BOOL bfnObtGeCtoPlan ( long *lGeCtoPlan,CTOPLAN **pstlGeCtoPlan,void* ctx)
{
	...
	while (iOra != NOT_FOUND)
	{
		...
		for (lCont = 0 ; lCont < lNumFilas ; lCont++)
		{
			strcpy(pstlGeCtoPlanTemp[lCont].szRowid,stlGeCtoPlanHost.szRowid[lCont]);
			...
			strcpy(pstlGeCtoPlanTemp[lCont].szNomUsuario,stlGeCtoPlanHost.szNomUsuario[lCont]);
...
BOOL bfnObtGePlanCtoPlan ( long *lGePlanCtoPlan,PLANCTOPLAN **pstlGePlanCtoPlan,void* ctx)
{
	...
	while (iOra != NOT_FOUND)
	{
		...
		for (lCont = 0 ; lCont < lNumFilas ; lCont++)
		{
			strcpy(pstlGePlanCtoPlanTemp[lCont].szRowid,stlGePlanCtoPlanHost.szRowid[lCont]);
			...
			strcpy(pstlGePlanCtoPlanTemp[lCont].szNomUsuario,stlGePlanCtoPlanHost.szNomUsuario[lCont]);
...
BOOL bfnObtGeCargosBasico ( long *lGeCargosBasico,CARGOSBASICO **pstlGeCargosBasico,void* ctx)
{
	int iOra;
	long lNumFilas;
	static CARGOSBASICO_HOST stlGeCargosBasicoHost;
	static CARGOSBASICO_HOST_NULL stlGeCargosBasicoHostNull;
	...
	iOra= ifnOraDeclararGeCargosBasico ( ctx);
	if (iOra != SQL_OK)
	{
	...
	while (iOra != NOT_FOUND)
	{
		iOra= ifnOraLeerGeCargosBasico(&stlGeCargosBasicoHost,&stlGeCargosBasicoHostNull,&lNumFilas,ctx);
		if (iOra != SQL_OK  && iOra != NOT_FOUND)
		{
			vfnLog(FILE_BOTH, "\n\nERROR(bfnObtGeCargosBasico):"
		...
		for (lCont = 0 ; lCont < lNumFilas ; lCont++)
		{
			pstlGeCargosBasicoTemp[lCont].iCodProducto = stlGeCargosBasicoHost.iCodProducto[lCont];
			strcpy(pstlGeCargosBasicoTemp[lCont].szCodCargoBasico,stlGeCargosBasicoHost.szCodCargoBasico[lCont]);
			strcpy(pstlGeCargosBasicoTemp[lCont].szFecDesde,stlGeCargosBasicoHost.szFecDesde[lCont]);
			if(stlGeCargosBasicoHostNull.sszFecHasta[lCont] != ORA_NULL)
				strcpy(pstlGeCargosBasicoTemp[lCont].szFecHasta,stlGeCargosBasicoHost.szFecHasta[lCont]);
			else
				strcpy(pstlGeCargosBasicoTemp[lCont].szFecHasta, "");
...
BOOL bfnObtGePlanTarif ( long *lGePlanTarif,PLANTARIF **pstlGePlanTarif,void* ctx)
{
	int iOra;
	long lNumFilas;
	...
	iOra= ifnOraDeclararGePlanTarif ( ctx);
	if (iOra != SQL_OK)
	{
	...
	while (iOra != NOT_FOUND)
	{
		...
		for (lCont = 0 ; lCont < lNumFilas ; lCont++)
		{
			pstlGePlanTarifTemp[lCont].iCodProducto = stlGePlanTarifHost.iCodProducto[lCont];
			...
			strcpy(pstlGePlanTarifTemp[lCont].szCod_Unidad,stlGePlanTarifHost.szCod_Unidad[lCont]);
			...
			strcpy(pstlGePlanTarifTemp[lCont].szFecDesde,stlGePlanTarifHost.szFecDesde[lCont]);
			if(stlGePlanTarifHostNull.sszFecHasta[lCont] != ORA_NULL)
				strcpy(pstlGePlanTarifTemp[lCont].szFecHasta,stlGePlanTarifHost.szFecHasta[lCont]);
			else
				strcpy(pstlGePlanTarifTemp[lCont].szFecHasta, "");			
			...
			long clieini,long cliefin)
{
	...
	*pstFacClientes = NULL;
	...
	iOra=ifnOraDeclararFacClientes(ctx,ciclo,clieini,cliefin);
	if (iOra != SQL_OK)
	{
	...
	while (iOra != NOT_FOUND)
	{
		...
		*pstFacClientes =(FAC_CLIENTES*) realloc(*pstFacClientes,(((*lFacClientes)+lNumFilas)*sizeof(FAC_CLIENTES)));
		...
		for (lCont = 0 ; lCont < lNumFilas ; lCont++)
		{
			pstFacClientesTemp[lCont].lRowNum =	0;
...
BOOL bfnObtFacCiclo( long *lFacCiclo,FAC_CICLO **pstFacCiclo,void* ctx,int ciclo,
		     long clieini, long cliefin)
{
	int    iOra;
	long   lNumFilasCiclo;
	static FAC_CICLO_HOST	stFacCicloHost;
	static FAC_CICLO_HOST_NULL  stFacCicloHostNull;
	FAC_CICLO *pstFacCicloTemp;
	long   lCont;

	*lFacCiclo = 0;
	*pstFacCiclo = NULL;

	vfnLog(FILE_LOG, "\n\t-Leyendo datos: FAC_CICLO");
	iOra=ifnOraDeclararFacCiclo(ctx,ciclo,clieini,cliefin);
	if (iOra != SQL_OK)
	{
		vfnLog(FILE_BOTH, "\n\nERROR(bfnObtFacCiclo):"
			"\n\tError al declarar cursor sobre FAC_CICLO[%d]",iOra);
		vfnImpErrorORACLE();
		return FALSE;
	}
	...
	while (iOra != NOT_FOUND)
	{
		...
		if (iOra != SQL_OK  && iOra != NOT_FOUND)
		{
			vfnLog(FILE_BOTH, "\n\nERROR(bfnObtFacCiclo):\n\tError al realizar el FETCH sobre FAC_CICLOCLI[%d]",iOra);
			vfnImpErrorORACLE();
			return FALSE;
		}
		if (!lNumFilasCiclo)
			break;
		*pstFacCiclo =(FAC_CICLO*) realloc(*pstFacCiclo,(((*lFacCiclo)+lNumFilasCiclo)*sizeof(FAC_CICLO)));
		if (!*pstFacCiclo)
		{
			vfnLog(FILE_BOTH,"\n\nERROR(bfnObtFacCiclo):\n\tError al alocar memoria\n\tError: [%d][%s]", errno, strerror(errno));
			return FALSE;
		...
		memset(pstFacCicloTemp, 0, sizeof(FAC_CICLO)*lNumFilasCiclo);
	...
	if (iOra != SQL_OK)
	{
		...
		return FALSE;
       	...
       	qsort((void*)*pstFacCiclo, *lFacCiclo, sizeof(FAC_CICLO),ifnCompareGeCiclo);
			  ...
			  long clieini, long cliefin,long iTipDireccion,long ciclo)
{
	...
	*pstlGeDirecciones = NULL;
	...
	iOra=ifnOraDeclararGeDirecciones2(ctx,iTipDireccion,ciclo,clieini,cliefin);
	if (iOra != SQL_OK)
	{
	...
	vfnLog(FILE_LOG, "\t\t\t[%d]", *lGeDirecciones);
	iOra=ifnOraCerrarGeDirecciones2(ctx,clieini,cliefin);
...
BOOL bfnObtGeOficina ( long *lGeOficina,OFICINA **pstlGeOficina,void* ctx)
{
  ...
  iOra= ifnOraContarPlaza (&iNumPlaza,ctx);
  
  if (iOra != SQL_OK)
	{
		vfnLog(FILE_BOTH, "\n\nERROR(ifnOraContarPlaza):"
		"\n\tError al declarar cursor sobre GE_CIUDADES");
		vfnImpErrorORACLE();
		return FALSE;
	}
	...
	{
		iOra= ifnOraDeclararGeOficina2 (ctx);
		if (iOra != SQL_OK)
		{
			vfnLog(FILE_BOTH, "\n\nERROR(bfnObtGeOficina2):"
			"\n\tError al declarar cursor sobre OFICINA");
			vfnImpErrorORACLE();
			return FALSE;
		}
		...
		while (iOra != NOT_FOUND)
		{
			iOra= ifnOraLeerGeOficina2(&stlGeOficinaHost,&lNumFilas,ctx);
			if (iOra != SQL_OK  && iOra != NOT_FOUND)
			{
    			vfnLog(FILE_BOTH, "\n\nERROR(bfnObtGeOficina2):"
    			"\n\tError al  realizar el FETCH sobre OFICINA");
		...
		iOra=ifnOraCerrarGeOficina2(ctx);
		if (iOra != SQL_OK)
		{
    		vfnLog(FILE_BOTH, "\n\nERROR(bfnObtGeOficina2):"
    		"\n\tError al cerrar el cursor");
...
/******************************************************************************
Funcion         :       ifnCompareGeConceptos
*******************************************************************************/
/*
static int ifnCompareGeConceptos(const void *pvstKey,const void *pvstItem)
{
	CONCEPTO *pstKey	= (CONCEPTO *) pvstKey;
	CONCEPTO *pstItem	= (CONCEPTO *) pvstItem;
	int rc = 0;
...
/*    		( (rc = pstKey->iCodProducto-
            		pstItem->iCodProducto ) != 0)?rc:
    		( (rc = strcmp ( pstKey->szCodActAbo,
                     	pstItem->szCodActAbo ) )!= 0)?rc:*/
...
static int ifnCompareGeCargosBasico(const void *pvstKey,const void *pvstItem)
{
	...
	int rc = 0;
  	...
  	return 
   		( (rc = pstKey->iCodProducto-
           		pstItem->iCodProducto ) != 0)?rc:
   		( (rc = strcmp ( pstKey->szCodCargoBasico,
                    	pstItem->szCodCargoBasico) ) != 0)?rc:
   		( (rc = strcmp ( pstKey->szFecDesde,
                    	pstItem->szFecDesde) ) != 0)?rc:0;
...
{
	FAC_CICLO *pstKey	 = (FAC_CICLO *) pvstKey;
	FAC_CICLO *pstItem       = (FAC_CICLO *) pvstItem;
	int rc = 0;
/* INICIO 86701 
  	if ( pstKey->lCodCliente > pstItem->lCodCliente )
       		rc =  1;
...
/* INICIO 86701 */
    return 
    ( (rc = pstKey->lCodCliente - pstItem->lCodCliente ) != 0)?rc:
    ( (rc = pstKey->lNumAbonado - pstItem->lNumAbonado ) != 0)?rc:0;
...
/******************************************************************************
Funcion         :       bfnObtCantConcep
*******************************************************************************/

BOOL bfnObtCantConcep ( long *lCantConc, void* ctx)
{
	int iOra;
	long lCont;

	if (ifnOraCuentaConcep (ctx, &lCont) != SQL_OK)
	{
		vfnLog(FILE_BOTH, "\n\nERROR(bfnObtCantConcep):"
		"\n\tError al consultar sobre FA_CONCEPTOS");
		vfnImpErrorORACLE();
		return FALSE;
	}
