...
BOOL attach_shared_ciclo(
	...
	CONCEPTO_MI *pstlGeConceptos_Mi,long *piNumConceptos_Mi,
...
/* 	DETPLANDESC **pstlGeDetPlanDesc, int *piNumDetPlanDesc, */
	DETCARG_PUNTUALES **pstlDescPuntuales, int *piNumDescPuntual,
	long *piNumFacCiclo)
{
	int iCont = 0;
  	int iError = 0;
...
/*	DETPLANDESC *pstDetPlanDescTemp=NULL; */
	DETCARG_PUNTUALES *pstDescPuntualesTemp=NULL;
	...
	*piNumDescPuntual=0;
	...
	if (iError != 0)
  	...
  	else
  	{
  		...
  		/* se prueba asignacion dinamica  */
		if (((*pstlGeConceptos) = (CONCEPTO *)malloc (sizeof (CONCEPTO) * TaS.lGeConceptos))== (CONCEPTO *)NULL)
	  	{
			fprintf(stderr,  "\n\nERROR(bfnAttachMemory): en asignacion de memoria estructura de CONCEPTOS");
	       	return FALSE;
	  	}
	  	else
	  	{  		
	...
	iError = ifnAttachMemory(ID_CARGOS, TaS.pDataTable,0,(void*)&TaS.pstlGeCargos,&TaS.lGeCargos);
  	...
  	iError = ifnAttachMemory(ID_FAC_CICLO, TaS.pDataTable,0,(void*)&TaS.pstlFacCiclo,&TaS.lFacCiclo);
  	if (iError != 0)
  	{
    		fprintf(stderr,  "\n\nERROR(bfnAttachMemory):"
      		       "\n\tError al mapear la memoria para la tabla %s"
      		       "\nError: [%s]", TaS.pDataTable[ID_FAC_CICLO].szTableName,
                      	strerror(iError));
    		return FALSE;
  	}
  	else
  	{
  		*piNumFacCiclo=TaS.pDataTable[ID_FAC_CICLO].lTotalRegistros;
  	}
...
*/	
	iError = ifnAttachMemory(ID_DETCARG_PUNTUALES, TaS.pDataTable,0,(void*)&TaS.pstlDescPuntuales,&TaS.lDescPuntuales);
  	if (iError != 0)
  	{
    		fprintf(stderr,  "\n\nERROR(bfnAttachMemory):"
      		       "\n\tError al mapear la memoria para la tabla %s"
      		       "\nError: [%s]", TaS.pDataTable[ID_DETCARG_PUNTUALES].szTableName,
                      	strerror(iError));
    		return FALSE;
  	}
  	else
  	{
        	*pstlDescPuntuales =(DETCARG_PUNTUALES*) realloc(*pstlDescPuntuales,(((*piNumDescPuntual)+TaS.lDescPuntuales)*sizeof(DETCARG_PUNTUALES)));
		if (!*pstlDescPuntuales)
		{
			fprintf(stderr,"\nError bfnCargaDescPuntuales: No se pudo reservar memoria.\n");
			return FALSE;
		}
		pstDescPuntualesTemp = &(*pstlDescPuntuales)[(*piNumDescPuntual)];
		memset(pstDescPuntualesTemp, 0, sizeof(DETCARG_PUNTUALES)*TaS.lDescPuntuales);
		for(iCont=0;iCont<TaS.pDataTable[ID_DETCARG_PUNTUALES].lTotalRegistros;iCont++)
  		{
  			pstDescPuntualesTemp[iCont]=TaS.pstlDescPuntuales[iCont];
  		}
  		*piNumDescPuntual=TaS.pDataTable[ID_DETCARG_PUNTUALES].lTotalRegistros;
  	}
...
BOOL bfnProcesarClientes(FAC_CLIENTES **pstCliente,FAC_CICLO *pstlFacCiclo,long *piNumAbonados,long piNumFacCiclo, int iSemId)
{
	static long iInd = -1;
	long i=0;
...
/*	               "\n\t\t*****De un total de [%d] Clientes\n", iInd, TaS.lFacClientes);*/
...
/*			fprintf(stderr,"\n\t\tCarga Clientes[i] en memoria.\n", iInd);                                            */
		    *pstCliente = &TaS.pstlFacClientes[iInd];
		    ...
		    while(i<piNumFacCiclo && bFinAbonados==FALSE)
		    {
...
*/
    vDTrazasLog ("","\n\t\t************* bCargaCargos_MC *************"
					...
					"\n\t\tlNumVenta       :[%d]"
					"\n\t\tlCargos         :[%d]"
					,LOG05, lCodCliente, piNumCargos,  lNumVenta, lCargos); 
	...
	if(lCargos!=-1)
	{
		...
		{
			vDTrazasLog (szExeName,"\n\t\t* TaS.pstlGeCargos[iInd].lCodCliente [%ld]  ",LOG05, TaS.pstlGeCargos[iInd].lCodCliente );   
			if(TaS.pstlGeCargos[iInd].lCodCliente==lCodCliente)
			{
				/*fprintf(stderr,"\n\t\tCarga Estructura pCargos[%d]", iCont);*/
			...
			{
				/*fprintf(stderr,"\n\t\tTotal de Cargos Recuperados:[%d] >Cambia de Cliente Estructura TaS.pstlGeCargos[%d]\n", iCont, iInd);*/
				bFinCargos=TRUE;
			}
			iInd++;
		}
