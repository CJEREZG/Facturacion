...
DECLARE
	...
	lv_FlagHostId			VARCHAR2(7);					/* Flag Host_id                                       */
...
BEGIN 
	...
	DBMS_OUTPUT.PUT_LINE ('lv_FlagHostId: '|| lv_FlagHostId);
	...
	IF lv_FlagHostId = 'HOST_ID' THEN
	    /* borra registros existentes para el ciclo */
	    BEGIN
	        DELETE FA_RANGOSHOST_TO WHERE COD_CICLFACT = ln_ciclfact;
	    EXCEPTION
	...
	ELSE
		...
		/* construye los comandos finales a ejecutar... */
		ln_aboandosrango := 0;
		DBMS_OUTPUT.PUT_LINE ('===================== / COMANDOS A EJECUTAR /============================');
		FOR i IN 1..indstRangos LOOP
	 		DBMS_OUTPUT.PUT_LINE ('facturaC -u / -s 1000000 -ciclo -f ' ||ln_ciclfact || ' -g '|| stRangos(i).COD_CLIENTE_DESDE || ' -h' || stRangos(i).COD_CLIENTE_HASTA || ' -l03');
			ln_aboandosrango := ln_aboandosrango + stRangos(indstRangos).CAN_ABONADOS;
		END LOOP;
		DBMS_OUTPUT.PUT_LINE ('===================== / COMANDOS A EJECUTAR /============================');
