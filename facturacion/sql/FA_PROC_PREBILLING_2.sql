/* ***************************************************************************************************** */
/* PROCEDIMIENTO : PREBILLING CONTADO 2  14-ENERO-2000                                              ROY! */
/* ***************************************************************************************************** */
CREATE OR REPLACE PROCEDURE FA_PROC_PREBILLING_2 ( 
	 lhCodCliente_EXT 		IN NUMBER  ,		/* Codigo del Cliente */
	 ...
	 /* lhNumCodModVenta_EXT	IN NUMBER  ,		 Mod.Venta - Se obtendrá desde Ga_Ventas */
...
BEGIN
	...
	DEBUG;
		...
		utl_file.put_line (fh,'>> Cod Modo Generacion    : [' || szhCodModGener_EXT    || ']' );
		utl_file.put_line (fh,'>> Cod Categ Tributaria   : [' || szhCodCaTribut_EXT    || ']' );
		utl_file.put_line (fh,'>> Fec Vencimiento Doc.   : [' || szhFecVencimiento_EXT || ']' );
...
EXCEPTION
	WHEN NO_DATA_FOUND THEN
		SET_ERROR('>> NO DATA FOUND (1.5)');
	WHEN OTHERS THEN
		SET_ERROR('>> ERROR INESPERADO (1.5)');
END GetPath;
...
BEGIN
	...
	SELECT  * 
	 ...
	 WHERE  NUM_VENTA = lhNumVenta_EXT	;
		...
		utl_file.put_line (fh,'>> Mod.Venta              : [' || Reg_GA_Ventas.Cod_ModVenta    || ']' );
		utl_file.put_line (fh,'>> Cod.Cuota para Número  : [' || Reg_GA_Ventas.Cod_Cuota       || ']' );
...
BEGIN
	...
	COMMIT;
	ProcesoCreado := TRUE; /*Se creó proceso en ejecución */
...
BEGIN
	...
	FOR i IN Tab_CARGA_CARGOS.FIRST .. Tab_CARGA_CARGOS.LAST LOOP
		/* Valores Externos (Recibidos por parámetros desde Visual Basic )*/
...
BEGIN
	...
	VP_ACT		:= 'N'				;
	DEBUG;
	status := fbGetCodGrpServicios (COD_CONCEPTO); /* DEL IMPUESTO */
	IF status = FALSE THEN
		RAISE ERROR_GET_COD_GRP_SERVI;
	ELSE
		status := fbGetPrcImpuesto ; 	/* AFECTO O EXENTO */
		IF status = FALSE THEN
			RAISE ERROR_GET_PRC_IMPUESTO;
		ELSE
			dhFCambio := 1;
			IF COD_MONEDA != szhCodMoneFact THEN
				status := fbConvierteMonedas ( SYSDATE,COD_MONEDA, szhCodMoneFact ); /* dhFCambio != 1 */
				IF status = FALSE THEN
					RAISE ERROR_CONVERSION_MONEDA ;
				END IF;
			END IF;
...
i BINARY_INTEGER;
j BINARY_INTEGER;
ihPrimerRegIva BINARY_INTEGER;
dhAcumNeto	 FA_PRESUPUESTO.IMP_FACTURABLE%TYPE ;
...
BEGIN
	...
	FOR i IN Tab_FA_PRESUPUESTO.FIRST .. FIN_DSCTO LOOP
		dhAcumNeto := dhAcumNeto + Tab_FA_PRESUPUESTO(i).IMP_FACTURABLE;
		j := Tab_FA_PRESUPUESTO.LAST + 1;  /* Agrego al final de la Tab_FA_PRESUPUESTO */
		/* Valores Externos (Recibidos por parámetros desde Visual Basic )*/
		Tab_FA_PRESUPUESTO(j).NUM_PROCESO	:= lhNumProceso_EXT			;
		Tab_FA_PRESUPUESTO(j).COD_CLIENTE	:= lhCodCliente_EXT			;
		...
		IF  Tab_FA_PRESUPUESTO(j).PRC_IMPUESTO = 0 THEN
			Tab_FA_PRESUPUESTO(i).FLAG_IMPUES  := 0	; /* EL CONCEPTO (RELACIONADO) ESTA EXENTO  */
		ELSE
	...
	/* Ajuste de IVA */
	utl_file.put_line (fh, Raya2);
	utl_file.put_line (fh, Asteriscos || ' Ajuste de IVA ***********');
	dhAcumIva := ROUND ((18/100) * dhAcumNeto);
	dhDifIva  := dhAcumIva - dhSumIva;
	IF dhDifIva != 0 THEN
		IF dhSumIva != 0 THEN
			utl_file.put_line (fh,'>> SE DEBE HACER AJUSTE DE IVA');
			utl_file.put_line (fh,'>> NETO TOTAL             : [' || dhAcumNeto|| ']' );
			utl_file.put_line (fh,'>> IVA TOTAL (18% NETO)   : [' || dhAcumIva|| ']' );
			utl_file.put_line (fh,'>> IVA ACUMULADO          : [' || dhSumIva|| ']' );
			utl_file.put_line (fh,'>> DIFERENCIA             : [' || dhDifIva|| ']' );
			
			<<LOOP_AJUSTE>>
			FOR j IN ihPrimerRegIva .. Tab_FA_PRESUPUESTO.LAST LOOP
				IF Tab_FA_PRESUPUESTO(j).IMP_FACTURABLE != 0 THEN
					Tab_FA_PRESUPUESTO(j).IMP_FACTURABLE := Tab_FA_PRESUPUESTO(j).IMP_FACTURABLE + dhDifIva;
					utl_file.put_line (fh, Raya1);
					utl_file.put_line (fh, '>> CONCEPTO RELACIONADO   : [' || Tab_FA_PRESUPUESTO(j).COD_CONCEREL || ']' );
					utl_file.put_line (fh, '>> NUEVO IMP_FACTURABLE   : [' || Tab_FA_PRESUPUESTO(j).IMP_FACTURABLE || ']' );
					EXIT;
				END IF;
			END LOOP LOOP_AJUSTE;
		ELSE
			utl_file.put_line (fh,'>> NO SE HACE AJUSTE DE IVA, PORQUE DOCUMENTO ES EXENTO');
		END IF;
	END IF;
...
BEGIN
	...
	DEBUG;
			...
			utl_file.put_line (fh,'>> NUMERO DE TRANSACCION  : [' || lhNumTransaccion_EXT  || ']' );
			utl_file.put_line (fh,'>> NUMERO DE PROCESO      : [' || lhNumProceso_EXT      || ']' );
			utl_file.put_line (fh,'>> NUMERO DE VENTA        : [' || lhNumVenta_EXT        || ']' );
			utl_file.put_line (fh,'>> IND. GENER. DOCUMENTOS : [' || szhCodModGener_EXT    || ']' );
			utl_file.put_line (fh,'>> DOCUMENTO A GENERAR    : [' || iDocumVenta           || ']' );
			utl_file.put_line (fh,'>> CATEG. TRIBUT. CLIENTE : [' || szhCodCaTribut_EXT    || ']' );
			utl_file.put_line (fh,'>> FOLIO DOC. RELACIONADO : [' || 'NULL'                || ']' );
			utl_file.put_line (fh,'>> EST. INICIAL PROCESO   : [' || iEstInicProc          || ']' );
			utl_file.put_line (fh,'>> ESTADO DEL PROCESO     : [' || iEstProceso           || ']' );
			utl_file.put_line (fh,'>> FEC VENCIMIENTO DOC.   : [' || szhFecVencimiento_EXT || ']' );
			
			utl_file.put_line (fh,'>> MOD. VENTA         .   : [' || Reg_GA_Ventas.Cod_ModVenta  || ']' );
			utl_file.put_line (fh,'>> NUMERO DE CUOTAS   .   : [' || lhNumCuotas           || ']' );
...
EXCEPTION
	WHEN UTL_FILE.WRITE_ERROR THEN
		SET_ERROR('Error de Escritura (F)(17)');
	WHEN UTL_FILE.INTERNAL_ERROR THEN
		SET_ERROR('Error Interno (?) (G)(17)');
	WHEN ERROR_NUEVO_PL THEN
		SET_ERROR('## ERROR EN LA INSERCION EN FA_INS_INTERFACT ');	
	WHEN OTHERS THEN
		SET_ERROR('>> ERROR INESPERADO (17)');
...
/* *********************** SELECCION DE PARAMETROS GENERALES Y DEL CLIENTE ****************** */
   ...
   IF (fallo = FALSE) THEN	
	...
	sControlPath := '1';
	DEBUG;
	utl_file.put_line(fh, Asteriscos || ' VERSION: ' || PROG_VERSION || '           *');
	utl_file.put_line(fh, Raya2);
...
EXCEPTION 
	...
	BEGIN
		ROLLBACK; /* Deshacer todas las acciones sobre la base*/
	...
	BEGIN
		IF sControlPath = '0' THEN
			VP_SQLERRM := 'Error al tratar de abrir archivo log, path = (' || szhPathDir || ')';
			VP_SQLCODE	:=  TO_CHAR(SQLCODE);
			VP_ERROR	:=  '1' ;		
		ELSE
	...
	BEGIN
		VP_SQLERRM := 'Error Registro en FAD_PARAMETRO (Reg.3) - Path  (S)';
		VP_SQLCODE	:=  TO_CHAR(SQLCODE);
		VP_ERROR	:=  '1' ;
