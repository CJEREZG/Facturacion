CREATE OR REPLACE PROCEDURE FA_PRE_NOTACREDPARCIAL(
	...
	sTabla := '';
  	...
  	gls_error := '';
        ...
        if vp_secuencia is null or vp_secuencia = '' then	   
        	vp_error := 1;
                gls_error := 'Error. Falta indentificar Secuencia N.C.Parcial en parámetros de entrada.';
                RAISE ERROR_PROCESO;
        end if;

        if vp_secuencia_ga is null or vp_secuencia_ga = '' then	   
        	vp_error := 2;
                gls_error := 'Error. Falta indentificar Secuencia GA.Abonados en parámetros de entrada.';
                RAISE ERROR_PROCESO;
        end if;
...
------- bValnotaCred
        ...
        IF vp_cod_modventa = 5 THEN
             	vp_error := 4;
                gls_error := 'Error. Factura de Venta Regalo. Tiene Restricción para realizar sólo N.C. Correctivas';
                RAISE ERROR_PROCESO;                        
        END IF;
