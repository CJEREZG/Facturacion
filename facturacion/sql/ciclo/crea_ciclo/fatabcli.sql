-- Se modifica los nombres de tablespaces por pruebas aranjuez folca (19-03-2003)
-- Se modifica tablespace para considerar posterior rename (JVELIZCA 19052005)
-- Se aplican modificaciones hechas en la operadora CH-200507052957
set echo off verify off
spool $WORKDIR/log/tab_FA_FACTCLIE_&1..log

define sCiclo= &1
column ciclo new_value numciclo noprint
select decode(length('&&sCiclo'),5,'0'||substr('&&sCiclo',1,1),6,substr('&&sCiclo',1,2),null) ciclo  from dual;
...
initrans  4
maxtrans  255
tablespace  "QTL_HCLIE_C&numciclo._scl001"
storage
(initial   7M
next       7M
minextents  1
...
initrans    8
maxtrans    255
tablespace   "QKL_HCLIE_C&numciclo._scl001"
storage
(initial    1M
next        1M
minextents  1
